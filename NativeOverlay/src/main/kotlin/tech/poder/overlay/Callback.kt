...
object Callback {
	...
	val init2 =
		NativeRegistry.loadLib(Paths.get("new.dll").toAbsolutePath())//, Paths.get("libWinAPILib.dll").toAbsolutePath())
	...
	*/
	...
	/*
	val getHNSActualDurationNative = NativeRegistry.register(
		FunctionDescription(
	...
	val stopRecordingNative = NativeRegistry.register(
		FunctionDescription(
			"StopRecording",
			//Int::class.java,
			params = listOf(
	...
	val startRecordingNative = NativeRegistry.register(
		FunctionDescription("StartRecording", MemoryAddress::class.java)
	)

	/*val getNextPacketSizeNative = NativeRegistry.register(
		FunctionDescription(
			"getNextPacketSize",
			Int::class.java,
			listOf(
	...
	val getBufferNative = NativeRegistry.register(
		FunctionDescription(
			"getBuffer",
			//Int::class.java,
			params = listOf(
	...
	val releaseBufferNative = NativeRegistry.register(
		FunctionDescription(
			"releaseBuffer",
			//Int::class.java,
			params = listOf(
	...
	/*
	val win32FromHResult = NativeRegistry.register(
		FunctionDescription(
			"Win32FromHResult",
			Int::class.java,
			listOf(
	...
	val recordSomething = NativeRegistry.register(
		FunctionDescription("recordSomething",
			Int::class.java)
	)
	*/
	...
	fun getService(client: MemoryAddress): MemoryAddress {
		...
		check(result >= 0) {
			"DeviceGetService failed: $result ${NativeRegistry[win32FromHResult].invoke(result)} ${NativeRegistry[getLastError].invoke()}"
		}
	...
	fun getActualDuration(format: MemoryAddress, bufferSize: UInt, REFTIMES_PER_SEC: Int = 1000000): Double {
		return NativeRegistry[getService].invoke(format, bufferSize.toInt(), REFTIMES_PER_SEC) as Double
	}
	...
	fun getPacket(service: MemoryAddress): NativeBuffer {
		...
		val result = NativeRegistry[getBuffer].invoke(service,
			pData.address(),
			numFramesAvailable.address(),
			flags.address()) as Int
		check(result >= 0) {
			"GetPacket failed: $result ${NativeRegistry[getLastError].invoke()}"
		}
		val res = NativeBuffer(MemoryAccess.getByte(pData),
	...
	fun startRecording() {
		NativeRegistry[startRecordingNative].invoke() as MemoryAddress
	}

	fun stopRecording() {
        NativeRegistry[stopRecordingNative].invoke() as MemoryAddress
    }

	/*
	fun getBuffer(): MemorySegment {

        val pData = MemorySegment.allocateNative(CLinker.C_POINTER.byteSize(), ResourceScope.newConfinedScope())
        val numFramesAvailable = MemorySegment.allocateNative(CLinker.C_INT.byteSize(), pData.scope())
        val flags = MemorySegment.allocateNative(CLinker.C_INT.byteSize(), pData.scope())

        val result = NativeRegistry[getBufferNative].invoke(
	        pData.address(),
            numFramesAvailable.address(),
            flags.address()
        ) as Int
